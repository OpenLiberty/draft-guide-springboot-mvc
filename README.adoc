...
= Running a Spring Boot MVC application as a war
...
Start by setting the application packaging type to liberty-assembly. No packaging type is specified in the starting project, so add `<packaging>liberty-assembly</packaging>` after the `<version>1.0-SNAPSHOT</version>` element in the POM.
